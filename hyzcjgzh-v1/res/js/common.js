...
function fmtSecondyyyMMddHHmmdot(second){
	return fmtDate(new Date(second*1000),"yyyy.MM.dd HH:mm");
}
...
//为 String 类增加一个 filterEmoji 方法
String.prototype.filterEmoji = function()
{
    // 用正则表达式去掉表情符号
    return this.replace(/[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF][\u200D|\uFE0F]|[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF]|[0-9|*|#]\uFE0F\u20E3|[0-9|#]\u20E3|[\u203C-\u3299]\uFE0F\u200D|[\u203C-\u3299]\uFE0F|[\u2122-\u2B55]|\u303D|[\A9|\AE]\u3030|\uA9|\uAE|\u3030/ig, "")
};
...
String.prototype.codeToBrRn = function()
{
	// 用正则表达式将前后空格用空字符串替代。
	return this.replace( /%3Cbr%3E/g, "\n" ).replace( /%3Cbr%3E/g, "\n" );
};
...
String.prototype.rnToBr = function()
{
    // 用正则表达式将前后空格用空字符串替代。
    return this.replace( /\r\n/g, "<br/>" ).replace( /\n/g, "<br/>" );
};
...
function filterEmoji(obj){
	var ipt = {};
	if(obj){
		ipt = $(obj).find("input,select,textarea")
	}else{
	...
	ipt.each(function(){
		var thet = $(this);
		//去除radio
		if("radio"==thet.attr("type") || "checkbox"==thet.attr("type")){
			return;
		}
		var value = thet.val();
		if(value){
			 thet.val(value.filterEmoji());
		}
	});
}
...
jQuery.fn.extend({
	paramsUrl: function() {
		var param="";
		var ipt = this.find("input,select,textarea");
		ipt.each(function(){
			var thet = $(this);
			//去除radio
			if("radio"==thet.attr("type") || "checkbox"==thet.attr("type")){
